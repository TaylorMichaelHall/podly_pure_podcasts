...
class TestConfigUpdateKeepsEnvOverlays:
    def test_put_config_reapplies_env_overlays(
        ...
        monkeypatch.setenv("LLM_MODEL", "env-model")
        monkeypatch.setenv("AD_CLASSIFIER_BACKEND", "jev")
        monkeypatch.setenv("JEV_API_KEY", "sk-or-v1-env")
        ...
        with app.app_context():
            ...
            response = app.test_client().put(
                "/api/config",
                json={"llm": {"openai_timeout": 99, "ad_classifier_backend": "llm"}},
            )

            assert response.status_code == 200
            assert runtime_config.openai_timeout == 99
            assert runtime_config.llm_model == "env-model"
            assert runtime_config.ad_classifier_backend == "jev"
            assert runtime_config.jev_api_key == "sk-or-v1-env"
